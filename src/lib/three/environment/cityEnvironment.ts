...
export class CityEnvironment {
  ...
  createGround() {
    ...
    const mainRoadGeometry = new THREE.PlaneGeometry(config.size * 0.9, 6);
    ...
    const crossRoadGeometry = new THREE.PlaneGeometry(6, config.size * 0.9);
  ...
  createStreetLights(): THREE.Group | null {
    ...
    const positions = config.positions && config.positions.length > 0 
      ? config.positions 
  ...
  setupLighting() {
    ...
    if (config.directional.enabled) {
      ...
      if (config.directional.shadowEnabled) {
        ...
        directionalLight.shadow.mapSize.width = config.directional.shadowResolution;
        directionalLight.shadow.mapSize.height = config.directional.shadowResolution;
